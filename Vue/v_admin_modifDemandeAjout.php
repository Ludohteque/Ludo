            ...
            <div class="col-sm-10">
                <select class="form-control" id="age" name="age" required="true">
                    ...
                    foreach ($tranches as $unetranche) {
                        ?>                      
                        <option value="<?php echo $unetranche->getIdAge(); ?>"<?php if ($ageActuel->getIdAge() == $unetranche->getIdAge()) {
...
foreach ($durees as $uneduree) {
    ?>                      
                        <option value="<?php echo $uneduree->getIdDuree(); ?>"<?php if ($dureeActuelle->getIdDuree() == $uneduree->getIdDuree()) {
        echo ' selected="selected"';
...
<?php foreach ($categories as $unecategorie) { ?>                      
                    <input type="checkbox" class="categories[]" name="categories[]" value="<?php $unecategorie ?>" <?php if (in_array($unecategorie, $jeu->getlesCategories())) {
        echo "checked";
...
foreach ($nbjoueurs as $unnbjoueur) {
    ?>                      
                        <option value="<?php echo $unnbjoueur[0]; ?>"<?php if ($nbjoueursActuel->getIdNbJoueurs() == $unnbjoueur[0]) {
        echo ' selected="selected"';
        ...
        <div class="form-group">
            ...
            <div class="col-lg-10">
                <input type="file" class="form-control heightfix" id="image" name="image" accept="image/*" value=\"Vue/img/jeu<?php if ($jeu->getImage()) {
                    echo $jeu->getImage();
                } ?>"><?php if ($jeu->getImage()) {
                    echo $jeu->getImage();
                } ?></input>
